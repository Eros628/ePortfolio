...
int main() {
    ...
    if (decision == 0) {
        ...
        if (!imagePath.empty()) {
            ...
            cv::Mat processedImage = getProcessedImage(image, net, confThreshold, nms Threshold);
            // Display the processed image with bounding boxes and labels
            cv::imshow("Object Detection", processed Image);
            cv::waitKey(0); 
        }
    }

    else if (decision == 1) {
        // Use the webcam to capture frames and perform object detection in real-time
        cv:: VideoCapture cap(0);
        while (cap.read(frame)) {
            cv::Mat processedImage2 getProcessedImage(frame, net, confThreshold, nms Threshold);
            // Display the processed image with bounding boxes and labels
            cv::imshow("Object Detection", processedImage2);
            if (cv::waitKey(30) == 27){
                break;
            }
        }
